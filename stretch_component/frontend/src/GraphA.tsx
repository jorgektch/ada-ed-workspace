...
const GraphA = (props:any) => {
    ...
    function aSearch(graph:Graph=graphMain){
      console.log("Empezando A Search")
      if(buttonRef.current){
      ...
      nodesMain.forEach((node)=>{
        distancesEnd[node.key] = (Math.sqrt(Math.pow(node.x - nodesMain[5].x,2) + Math.pow(node.y - nodesMain[5].y,2)));
        visited.push(false);
        ...
        txN.fill("#d9b420");
        let txNDs = createLetterCoords(2*radius*node.x + radius,2*radius*(node.y)-radius/2,drawRef.current,myTime,3*radius/4,3*radius/4,theme);
        txNDs.fill("#65f057").text(distancesEnd[node.key].toFixed(2).toString());
        listSvg.push(txN)
        listSvg.push(txNDs)
        txNodes.push(txN);
      })
      ...
      while(queue.length > 0){  
        let st:string = '';
        queue.forEach((el)=>{
          st = st + ", " + el[0].toFixed(2) + "-" + el[1].value;
        })
        console.log("Cola",st);
        console.log("Distancias",distances);
        console.log("Recorrido?",visited);
        ...
        console.log("Nodo actual",currentNode.value);

        visited[currentNode.key] = true;
        if(currentNode.key == nodesMain[5].key){
          console.log("The destination cell is found\n");
          break;
        } 

        const svgAux:List<Line>= new List();
        graphMain.adjList.get(currentNode)?.forEach((edge)=>{
          let nodeB:Node;
          ...
          if(!visited[nodeB.key] && distances[currentNode.key] != Infinity){
            ...
            let g = edge.weight + distances[currentNode.key];
            let h = distancesEnd[nodeB.key];
            let f = g + h;
            if(distances[nodeB.key] > g){
    ...
    return (
      <>
        ...
        <button ref = {buttonRef} onClick={()=>{aSearch()}} onMouseOver={handleButtonOver} onMouseOut={handleButtonOut} style={buttonOut} >A*</button>       
      </>
    );
  };
